...
Future<bool> updateStoreAndState(Ref ref) async {
  ...
  client.interceptors.add(QueuedInterceptorsWrapper(
    onError: (error, handler) async {
      ...
      if (hasNewTokens) {
        try {
          ...
          print('Old: ${authResponse.accessToken}');
          print(
              'New: ${ref.watch(sharedPreferencesProvider).getString(localStorageAccessTokenKey)!}');
    ...
    onRequest: (options, handler) {
      print('Sending Token:${authResponse.accessToken}');
